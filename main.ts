...
function extractStyleDefs(styles: any, map: Map<string, string>) {
    let result = []
    for (let style of getChildTag(styles, "w:styles")["w:styles"]) {
        if (!style["w:style"]) continue
...
function patchStyleDefs(styles: any[], map: Map<string, string>) {
    let result = []
    for (let style of styles) {
        if (map.has(style[":@"]["w:styleId"])) {
            ...
            let basedOnTag = getChildTag(style["w:style"], "w:basedOn")
            if (basedOnTag) basedOnTag[":@"]["w:val"] = map.get(basedOnTag[":@"]["w:val"])

            let linkTag = getChildTag(style["w:style"], "w:link")
            if (linkTag) linkTag[":@"]["w:val"] = map.get(linkTag[":@"]["w:val"])

            let nextTag = getChildTag(style["w:style"], "w:next")
            if (nextTag) nextTag[":@"]["w:val"] = map.get(nextTag[":@"]["w:val"])
